...
import { Edit2, Trash2, Plus, X, Check, Package, TrendingUp, Users, Info, Settings, BadgePercent } from 'lucide-react';
...
export default function AdminBusinessPackagesPage() {
  ...
  return (
    <div className="p-6 lg:p-10 bg-gray-50/30 min-h-screen">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-5">
                ...
                <div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cəmi Gəlir (AZN)</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {packages.map(pkg => (
              <div key={pkg.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all group flex flex-col relative overflow-hidden">
                 ...
                 <div className="bg-slate-50/50 rounded-3xl p-6 mb-8 border border-slate-50">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Baza Qiymət (30 GÜN)</p>
                    <p className="text-3xl font-black text-slate-900">{pkg.basePrice.toFixed(0)} <span className="text-lg">AZN</span></p>
                 </div>

                 <div className="flex-1 space-y-4 mb-8">
                    {[
                        { label: 'Elan Limiti', val: pkg.adLimit, icon: Check, color: 'green' },
                        { label: 'Xidmət Balansı', val: `${pkg.serviceBalance} AZN`, icon: Wallet, color: 'blue' },
                ...
                <tbody className="divide-y divide-slate-50">
                   {userPackages.map(up => (
                     <tr key={up.id} className="text-sm group hover:bg-slate-50/50 transition-all font-medium text-slate-600">
                        ...
                        <td className="px-8 py-5 font-black text-primary">{up.price.toFixed(2)} AZN</td>
                        <td className="px-8 py-5">
      ...
      {/* Styled Icons Helper (Using lucide-react, assuming typical project setup has it) */}
    </div>
  );
}

const Wallet = ({ size, className }: any) => <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M19 7V4a1 1 0 0 0-1-1H5a2 2 0 0 0 0 4h15a1 1 0 0 1 1 1v4h-3a2 2 0 0 0 0 4h3a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1"/><path d="M3 5v14a2 2 0 0 0 2 2h15a1 1 0 0 0 1-1v-4"/><circle cx="14" cy="15" r="1"/></svg>;
