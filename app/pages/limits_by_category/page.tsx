...
export default function LimitsByCategory() {
  ...
  const filteredCategories = categories.filter((cat: CategoryDto) => {
    ...
    const matchesParent = name.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesChild = cat.children?.some((child: CategoryDto) => {
      const childName = language === 'ru' && child.nameRu ? child.nameRu : child.name;
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 flex flex-col gap-3">
          ...
          <div>
            <h3 className="font-bold text-blue-900 text-sm mb-1">{t('limits.periodTitle')}</h3>
            <p className="text-xs text-blue-700/70 font-medium leading-relaxed">{t('limits.periodDesc')}</p>
          </div>
        </div>
        <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 flex flex-col gap-3">
          ...
          <div>
            <h3 className="font-bold text-emerald-900 text-sm mb-1">{t('limits.packagesTitle')}</h3>
            <p className="text-xs text-emerald-700/70 font-medium leading-relaxed">{t('limits.packagesDesc')}</p>
          </div>
        </div>
        <div className="bg-amber-50/50 p-5 rounded-2xl border border-amber-100 flex flex-col gap-3">
          ...
          <div>
            <h3 className="font-bold text-amber-900 text-sm mb-1">{t('limits.vipTitle')}</h3>
            <p className="text-xs text-amber-700/70 font-medium leading-relaxed">{t('limits.vipDesc')}</p>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="px-6 py-4 text-[11px] font-black text-gray-500 uppercase tracking-widest sticky left-0 bg-gray-50 z-10">{t('limits.table.category')}</th>
                  <th className="px-4 py-4 text-[11px] font-black text-gray-500 uppercase tracking-widest text-center border-l border-gray-100">{t('limits.table.limit')}</th>
                ...
                <tr className="bg-gray-50/50 border-b border-gray-100 text-[10px] font-black text-gray-400">
                  ...
                  <th className="px-4 py-2 text-center border-l border-gray-100">30 {t('cabinet.dayShort')}</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredCategories.map((parent: CategoryDto) => (
                  <Fragment key={parent.id}>
                    <tr className="bg-gray-50/30">
                      <td colSpan={11} className="px-6 py-3 text-gray-900 font-extrabold text-xs uppercase tracking-wider">
                        {language === 'ru' && parent.nameRu ? parent.nameRu : parent.name}
                      </td>
                    </tr>
                    {parent.children?.map((child: CategoryDto) => (
