...
export default function UserSidebar() {
  ...
  const user = {
    name: authUser?.fullName || (authUser ? 'İstifadəçi Adı' : 'Qonaq'),
    email: authUser?.email || (authUser ? 'istifadeci@email.com' : 'Giriş etməmisiniz'),
          ...
          {isAuthenticated ? (
            <>
              <div className="hidden sm:flex lg:flex flex-col lg:p-4 bg-gray-50 rounded-xl border border-gray-100 lg:mt-2 px-4 py-2 flex-shrink-0 text-right lg:text-left">
                <p className="text-gray-500 text-[10px] font-semibold uppercase tracking-wider mb-1">Balansınız</p>
                <div className="flex items-center justify-end lg:justify-between gap-3">
                  <span className="text-gray-900 text-base lg:text-xl font-bold">{user.balance.toFixed(2)} <span className="text-[10px] lg:text-sm font-medium text-gray-400">AZN</span></span>
                  <button
              ...
              <div className="sm:hidden flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex flex-col">
                    <span className="text-gray-500 text-[10px] font-semibold uppercase tracking-wider mb-0.5">Balans</span>
                    <span className="text-gray-900 text-lg font-bold">{user.balance.toFixed(2)} AZN</span>
                </div>
                <button
                    onClick={() => setIsTopUpModalOpen(true)}
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 px-3 lg:px-4 py-2 lg:py-2.5 rounded-xl transition-all whitespace-nowrap ${
                    isActive
                ...
                >
                  <span
                    className={`material-symbols-outlined !text-[20px] lg:!text-[22px] ${isActive ? 'text-primary' : 'text-gray-500'}`}
                    style={item.filled && isActive ? { fontVariationSettings: "'FILL' 1, 'wght' 400" } : undefined}
                  >
                    {item.icon}
                  </span>
                  <p className={`text-xs lg:text-sm font-medium ${isActive ? 'text-primary font-bold' : ''}`}>
                    {item.label}
                  </p>
                </Link>
              );
