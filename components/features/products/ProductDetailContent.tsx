      ...
      try {
        ...
        if (data.isStore && data.storeId) {
          try {
            ...
            const transformed = (ads || []).filter((ad: any) => ad.id !== data.id && ad.pinCode?.toString() !== data.pinCode?.toString()).slice(0, 8).map((ad: any) => {
              ...
              return {
                ...
                category: { name: language === 'ru' && ad.categoryRu ? ad.categoryRu : (ad.categoryName || ad.category || ''), slug: ad.parentCategorySlug || '' },
        ...
        if (data.categoryId) {
          ...
          adService.getVipAds({ categoryId: data.categoryId, pageNumber: 1, pageSize: 12 })
            .then(ads => {
              const transformed = (ads || []).filter(ad => ad.id !== data.id && ad.pinCode?.toString() !== data.pinCode?.toString()).slice(0, 4).map((ad: any) => {
                const imageUrl = ad.image ? getImageUrl(ad.image) : null;
                return {
                  id: ad.id.toString(),
                  title: ad.title,
                  ...
                  currency: '₼',
                  images: imageUrl ? [imageUrl] : [],
                  category: { name: language === 'ru' && ad.categoryRu ? ad.categoryRu : ad.category, slug: ad.parentCategorySlug },
                  subCategory: ad.childCategorySlug ? { name: language === 'ru' && ad.subCategoryRu ? ad.subCategoryRu : (ad.subCategory || ''), slug: ad.childCategorySlug } : undefined,
                  location: { city: ad.city ?? '', cityRu: ad.cityRu },
                  createdAt: new Date(ad.createdDate),
          ...
          adService.getAllAds({ categoryId: data.categoryId, pageNumber: 1, pageSize: 10 })
            .then(res => {
              const ads = res.data || [];
              const filtered = ads.filter(ad => ad.id !== data.id && ad.pinCode?.toString() !== data.pinCode?.toString() && !ad.isVip).slice(0, 8);
              ...
              <div className="flex flex-col gap-3 flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    {product.isPremium && (
                      <PremiumIcon size={28} title="Premium" className="shrink-0" />
                    )}
                    {product.isVip && (
                      <VipIcon size={24} title="VIP" className="shrink-0" />
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-y-4 pt-6 border-t border-gray-50">
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{t('product.adNumber')}: <span className="text-gray-900 font-semibold">{product.pinCode || product.id.slice(0, 8)}</span></span>
                      </div>
                      <div className="flex items-center gap-1">
